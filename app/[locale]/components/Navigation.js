  ...
  return (
    <nav className="z-10 text-xl hidden lg:flex flex-grow justify-center items-center h-full">
      <ul className="flex items-center text-lg pt-4 text-neutral-900 space-x-2 h-full">
        <li className="list-none h-full flex items-center text-nowrap">
        ...
        {navOptions.slice(1).map((option, index) => (
          <li
            key={index}
            className="list-none h-full flex items-center text-nowrap"
          >
            <Link href={`/${locale}${option.url}`}>
              <div className="px-4 py-2 text-nowrap transition-all lg:text-[18px]  duration-300 2xl:text-xl hover:text-red-400 rounded-xl">
                {option.name}
              </div>
            </Link>
          </li>
