...
export default function ServiceItemBanner({ service, locale }) {
  ...
  const serviceName =
    service.name?.[locale] || service.name?.ru || 'Название сервиса'
  ...
  const isBlockContent = content => {
    return (
      Array.isArray(content) && content.some(block => block._type === 'block')
    )
  }
      ...
      <div className='w-full max-w-[1440px] flex flex-col px-2 gap-5 mx-auto'>
        ...
        <div className='flex gap-3 max-md:flex-wrap'>
          <button
            ...
            className='mdx:px-10 px-4 py-3 rounded-full bg-red-400 text-white font-semibold'
          >
            {t('online-application')}
          </button>
          <button
            onClick={() => setContactWithUs(true)}
            className='mdx:px-10 px-6 py-3 rounded-full border border-red-400 text-red-400 font-semibold'
          >
            {t('connect')}
          </button>
        </div>
